# Script to probe and find compatible scripting API versions for the Bedrock Dedicated Server (without server-ui first).
...
# Comprehensive candidate versions for @minecraft/server to check
$serverApiVersions = @(
    # Stable versions
    "1.8.0", "1.9.0", "1.10.0", "1.11.0", "1.12.0", "1.13.0", "1.14.0", "1.15.0", "1.16.0", "1.17.0",
    # Beta versions (1.x)
    "1.8.0-beta", "1.9.0-beta", "1.10.0-beta", "1.11.0-beta", "1.12.0-beta", "1.13.0-beta", "1.14.0-beta", "1.15.0-beta", "1.16.0-beta", "1.17.0-beta",
    # Beta versions (2.x)
    "2.0.0-beta", "2.1.0-beta", "2.2.0-beta", "2.3.0-beta", "2.4.0-beta", "2.5.0-beta", "2.6.0-beta", "2.7.0-beta", "2.8.0-beta",
    # 3.0 alpha
    "3.0.0-alpha"
)
...
Write-Host "Starting @minecraft/server version compatibility test..." -ForegroundColor Cyan
...
foreach ($serverVer in $serverApiVersions) {
    ...
    while ($retryCount -le $maxRetries) {
        Write-Host "Testing @minecraft/server: $serverVer (Attempt $($retryCount + 1))..." -ForegroundColor Yellow
        ...
        $logFile = Join-Path $scriptPath "test_run_${safeServerVer}_${retryCount}.log"
        $errFile = Join-Path $scriptPath "test_err_${safeServerVer}_${retryCount}.log"
        ...
        # Wait 3.5 seconds for server initialization and script compilation
