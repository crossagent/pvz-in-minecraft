...
- **Rewrite Legacy Logic Instead of Compatibility Layers**: Do not preserve incompatible legacy `mcfunction` behavior by adding compatibility wrappers. When old logic conflicts with the current script-first runtime, add focused tests for the affected behavior, rewrite it in the current JavaScript system, and then update or remove the legacy hook.
